...
% Last update: 06/2026 - correction for asymmetry + extension to Norther Hemisphere
% Note make sure LAT is negative for SH, positive for NH
%__________________________________________________________________________
...
OutputFileName = 'GBR_cyclones_MSW_2008-2026_NEW';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% UPDATE 06/2026: add parameter for asymmetry in Southern vs Northern Hemisphere
% Previous version was working for SH only
% NH -> strongest winds in the right-front quadrant
% SH -> strongest winds in the left-front quadrant
                ...
                if count~=0 % count=0 if there was no recorded value for the radius of maximum wind -> just ignore this cyclone
                    ...
                    for n = 1:length(MY_LOCATIONS.Reef_ID)
                        ...
                        for t = 2:size(CURRENT_CYCLONE_TRACK,1) % for every time position on the track
                            ...
                            % Alternative (06/2026):
                            dtheta = AZ_CR - AZ_CT; % relative angle reef vs motion
                            dtheta = mod(dtheta + 180, 360) - 180; % wrap to [-180, 180]
                            theta = deg2rad(90 - dtheta); % convert to mathematical CCW radians
                            TRACK.THETA(t,1) = theta;                                  
                        ...
                        if isempty(select_dist)==0 % (if empty will loop to the next reef)
                            ...
                            switch ModelChoice
                                ...
                                case "Holland"
                                    ...
                                    % Symmetric part (Holland vortex) - this is the estimated 10-min MSW on the reef
                                    ...
                                    % Add the effect of cyclone motion (asymmetry) following McConochie et al 1999
                                    % SELECTION.alpha = 0.5*(1+cos(deg2rad(SELECTION.THETA-65)));
                                    % SELECTION.VR = SELECTION.VR + SELECTION.alpha.*SELECTION.VH;
                            
                                    % Alternative (global approach SH+NH)
                                    sH = - sign(SELECTION.LAT); % positive in SH, negative in NH
                                    SELECTION.alpha = 0.5*(1+cos(deg2rad(sH.*SELECTION.THETA-65)));
                                    SELECTION.VR = SELECTION.VR + SELECTION.alpha.*SELECTION.VH;
